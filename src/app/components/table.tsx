...
import { ArrowBigDown, ArrowDown, ChevronDown, Hash, Plus } from "lucide-react";
...
export default function TableDisplay() {
  const initialColumns: ColumnDef<Data>[] = [
    { accessorKey: "col_1", header: "Column 1", cell: ({ getValue }) => <Input type="text" className="w-full h-full focus:border-[#176EE1] pl-3 rounded-none border-w focus:border-solid focus:border-[2px]" value={getValue() as string} /> },
    { accessorKey: "col_2", header: "Column 2", cell: ({ getValue }) => <Input type="text" className="w-full h-full focus:border-[#176EE1] pl-3 rounded-none border-none focus:border-solid focus:border-[2px]" value={getValue() as string} /> },
    { accessorKey: "col_3", header: "Column 3", cell: ({ getValue }) => <Input type="text" className="w-full h-full focus:border-[#176EE1] pl-3 rounded-none border-none focus:border-solid focus:border-[2px]" value={getValue() as string} /> },
    { accessorKey: "col_4", header: "Column 4", cell: ({ getValue }) => <Input type="text" className="w-full h-full focus:border-[#176EE1] pl-3 rounded-none border-none focus:border-solid focus:border-[2px]" value={getValue() as string} /> },
  ];
  ...
  const addColumn = (fieldName: string, fieldType: string) => {
    ...
    const newColumn: ColumnDef<Data> = {
      ...
      cell: ({ getValue }) => fieldType === 'number' ? <input type="number" value={getValue() as number} /> : <input type="text" value={getValue() as string} />
    };
            ...
            <TableRow key={headerGroup.id} className="hover:bg-transparent bg-gray-100 bg-red">
              {headerGroup.headers.map((header) => (
                <TableHead key={header.id} style={{ width: cellWidth }} className="h-8 border border-gray-300 ">
                  <div className="flex felr-row justify-between items-center">
